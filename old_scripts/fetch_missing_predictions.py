...
MAX_REQUESTS_PER_RUN = 10  # stay within free plan daily limit

def get_fixtures_missing_predictions(limit=20):
    """Return future fixtures that have no prediction record."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE p.id IS NULL
                AND f.date > NOW() - INTERVAL '1 day'   # include recent past as well
                ORDER BY f.date ASC
                LIMIT %s
            """, (limit,))
            return cur.fetchall()
...
def fetch_prediction(api_fixture_id, fixture_db_id):
    ...
    try:
        ...
        probability = predictions.get("win_probability")  # could be None
...
def main():
    ...
    for i, fix in enumerate(fixtures):
        ...
        if result:
            prob_str = f"{result['probability']*100:.1f}%" if result['probability'] is not None else "N/A"
            print(f"    ✅ Stored prediction: {result['winner']} ({prob_str})")
        else:
